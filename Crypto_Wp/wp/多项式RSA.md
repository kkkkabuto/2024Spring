...
- 关键在于求phi，对于多项式P(y)，其欧拉函数phi(P(y))的定义是：
对于多项式P(y)来讲，欧拉函数phi(P(y))表示所有不高于P(y)幂级的环内所有多项式中，与P(y)无（除1以外）公因式的其他多项式的个数。
- 由题目给的代码可知：P(y)经过了is_irreducible判断，所以P(y)是不可约多项式。所以 phi(P(y)) = (43753^degree()-1)；Q(y)同理。就是多项式的最高项指数
- 解出欧拉函数后，就是常规RSA解法了。
最后的输出结果注意一下是多项式形式的，因为加密信息的原理就是，把明文的每个字符都转化成数值，作为多项式上的系数。只需要逆向输出一下就ok了
- 在sage中运行
- EXP：
```python
#sage
